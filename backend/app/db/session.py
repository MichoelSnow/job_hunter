    ...
    with engine.begin() as conn:
        ...
        if "passes_user_filters" not in columns:
            ...
            conn.exec_driver_sql(
                "ALTER TABLE jobs ADD COLUMN passes_user_filters BOOLEAN DEFAULT 1"
            )
        conn.exec_driver_sql(
...
def _ensure_sqlite_user_settings_columns() -> None:
    ...
    with engine.begin() as conn:
        ...
        if "user_settings" not in tables:
            return
        columns = {
            row[1] for row in conn.exec_driver_sql("PRAGMA table_info(user_settings)").fetchall()
        }
        if "filter_target_salary" not in columns:
            logger.info("Adding missing user_settings.filter_target_salary column")
            conn.exec_driver_sql(
                "ALTER TABLE user_settings ADD COLUMN filter_target_salary INTEGER"
            )
        ...
        conn.exec_driver_sql(
            "UPDATE user_settings SET matching_skills = '[]' WHERE matching_skills IS NULL"
        )
        ...
        select_columns = [
            ...
            if has_filter_title_keywords
            else "NULL AS filter_title_keywords",
        ]

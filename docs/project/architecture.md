...
Incoming location text is preserved in `jobs.location_raw`, while `jobs.location` stores a canonical display value used by the UI. Generic NYC aliases display as `NY, NY`; specific boroughs retain their names with `, NY`; state-only locations display as the full state name; and remote-only locations display as `Remote`. Structured `job_locations` records store each city, state, and country for filtering. The Jobs filter exposes only populated groups: `NYC` (including its boroughs), `NY State (not NYC)`, each other U.S. state, `Outside US`, `Remote`, and `Unknown`. A job may belong to multiple geographic groups when it lists multiple geographic locations. Arrangement markers remain in `work_arrangement` and are not treated as geographic locations. Unrecognized or placeholder values such as `None, None` normalize to an empty location and remain available through `location_raw`.
